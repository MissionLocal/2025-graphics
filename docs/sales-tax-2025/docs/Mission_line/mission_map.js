document.addEventListener("DOMContentLoaded", () => {
  ...
  const PALETTES = {
    ...
    crimson_emerald: ["#D32F2F", "#C62828", "#FAFAFA", "#43A047", "#2E7D32"],
  };
  ...
  const v2025 = props["2025adj"];
  const name = props.corridor || "Commercial corridor";
  ...
  return `
      <div class="info-desc">
        ...
        <div>Sales tax revenue in 2025 Q1: <strong>${fmtMoney(
          v2025
        )}</strong></div>
      </div>
    `;
  ...
  map.on("load", () => {
    fetch("mission_corridor_lines.geojson")
      .then((r) => r.json())
      .then((data) => {
        ...
        // halo / glow under the main line
        map.addLayer(
          {
            id: "mission-line-halo",
            type: "line",
            source: "mission",
            paint: {
              "line-color": "#333333",
        ...
        map.addLayer({
          ...
          paint: {
            "line-color": "#000",
            "line-width": ["interpolate", ["linear"], ["zoom"], 11, 7, 14, 12],
            "line-opacity": 0.9,
          },
        });
        ...
        map.addLayer({
          ...
          paint: {
            "line-color": "#000",
            "line-width": ["interpolate", ["linear"], ["zoom"], 11, 6, 14, 11],
            "line-opacity": 0.4,
          },
        });
        ...
        if (paletteSelect) {
          paletteSelect.addEventListener("change", (e) => {
            ...
            map.setPaintProperty("mission-line", "line-color", newExpr);
            updateLegendGradient(currentPalette);
          });
        }

        map.on("mousemove", "mission-line", (e) => {
        ...
        map.on("mouseleave", "mission-line", () => {
          map.setFilter("mission-hover", ["==", ["get", UID_FIELD], ""]);
          map.getCanvas().style.cursor = "";
        });

        map.on("click", "mission-line", (e) => {
          if (!e.features?.length) return;
